...
labelArray.append(" and so on")
cutOffArray = [8000,15000,22000,25000,27000]
doFit = []
lowFitLimits = []
upFitLimits = []
...
for iteration in range(len(fileArray)):
    ...
    for i in range(len(totXVals[iteration])):
        if totXVals[iteration][i] > cutOffArray[iteration]:
            upper_limit = i
            break
    ...
    plt.plot(totXVals[iteration][:upper_limit],np.multiply(totYVals[iteration][:upper_limit],-1),label=str((labelArray[iteration])))
    negyVals = np.multiply(totYVals[iteration][:upper_limit],-1.0)
    #print np.divide(yErrs,yVals)
    plt.figure(1)
    plt.plot(totXVals[iteration][:upper_limit],np.divide(totYErr[iteration][:upper_limit],negyVals),label="Error over area: "+str(labelArray[iteration]))
    if doFit[iteration]:
        plt.figure(0)
        fitWeights = []
        ...
        fitCounter += 1
        fitResults = np.polyfit(totXVals[iteration][lowIndex:upIndex],np.multiply(totYVals[iteration][lowIndex:upIndex],-1.0),1,full=True)
        ...
        print "X axis intercept is: "+str(-fitValues[1]/fitValues[0])
